...
export default function ProjectionScreen({ navigation }: Props) {
  ...
  return (
    <Screen>
      <Text style={styles.title}>In one year…</Text>
      <Text style={styles.subtitle}>A clear picture of what you’re gaining.</Text>
      ...
      <View style={styles.heroCard}>
        ...
        <Text style={styles.heroLabel}>cigarettes avoided</Text>
      </View>

      <Text style={styles.note}>
        Imagine what you could do with that. You’ve already started.
      </Text>

      <View style={{ flex: 1 }} />

      <PrimaryButton title="Continue" onPress={() => navigation.navigate("Paywall")} />
      <Text style={styles.tiny}>Quit date: {quitDate}</Text>
    </Screen>
  );
}
